...
def _mocked_beacon_node_endpoints(
    ...
    def _mocked_beacon_api_endpoints_get(url: URL, **kwargs: Any) -> CallbackResult:
        ...
        if re.match("/eth/v3/validator/blocks/.*", url.raw_path):
            ...
            block_cls_map = {
                ...
                ForkVersion.FULU: SpecBeaconBlock.FuluBlockContents,
            }
            blinded_block_cls_map = {
                ForkVersion.ELECTRA: SpecBeaconBlock.ElectraBlindedBlock,
                ForkVersion.FULU: SpecBeaconBlock.FuluBlindedBlock,
            }
    ...
    def _mocked_beacon_api_endpoints_post(url: URL, **kwargs: Any) -> CallbackResult:
        ...
        if re.match("/eth/v2/beacon/blocks", url.raw_path):
            ...
            if fork_version == ForkVersion.ELECTRA:
            ...
            elif fork_version == ForkVersion.FULU:
                if headers["Content-Type"] == ContentType.JSON.value:
                    _ = SpecBeaconBlock.FuluBlockContentsSigned.from_obj(
                        msgspec.json.decode(kwargs["data"].decode())
                    )
                else:
                    _ = SpecBeaconBlock.FuluBlockContentsSigned.decode_bytes(
                        kwargs["data"]
                    )
        ...
        if re.match("/eth/v2/beacon/blinded_blocks", url.raw_path):
            ...
            if fork_version == ForkVersion.ELECTRA:
                ...
                if headers["Content-Type"] == ContentType.JSON.value:
                    _ = SpecBeaconBlock.FuluBlindedBlockSigned.from_obj(
                        msgspec.json.decode(kwargs["data"].decode())
                    )
                else:
                    _ = SpecBeaconBlock.FuluBlindedBlockSigned.decode_bytes(
                        kwargs["data"]
                    )
